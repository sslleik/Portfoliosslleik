class TGKAnalytics {
  ...
  async getBotInfo() {
    ...
    try {
      ...
      if (result.ok) {
        console.log('%c✅ Telegram Bot подключен: - analytics.js:55' + result.result.first_name, styles.success);
        if (this.chatId) {
          console.log('%c✅ Chat ID настроен: - analytics.js:57' + this.chatId, styles.success);
          console.log('%c✨ Аналитика будет отправляться в Telegram! - analytics.js:58', styles.success);
        } else {
          console.warn('%c⚠️ Chat ID не настроен! - analytics.js:60', styles.warning);
          console.warn('%c📌 Чтобы включить аналитику: - analytics.js:61', styles.warning);
          console.log('%c1. Откройте %canalyticsconfig.js - analytics.js:62', styles.info, styles.code);
          console.log('%c2. Найдите строку: %cCHAT_ID: null, - analytics.js:63', styles.info, styles.code);
          console.log('%c3. Получите свой ID у бота: %c@userinfobot - analytics.js:64', styles.info, styles.code);
          console.log('%c4. Замените null на ваш ID (например): %cCHAT_ID: 123456789, - analytics.js:65', styles.info, styles.code);
          console.log('%c📖 Подробные инструкции в файле: %cANALYTICS_SETUP.md - analytics.js:66', styles.info, styles.code);
        }
      } else {
        console.error('%c❌ Ошибка подключения к боту - analytics.js:69', styles.error);
        console.error(result);
      }
    } catch (e) {
      console.warn('%c⚠️ Не удалось подключиться к боту - analytics.js:73', styles.warning);
      console.warn(e.message);
    }
  }
  async init() {
    console.log('🚀 TGK Analytics starting... - analytics.js:78', this.sessionID);
    ...
    this.trackPageView();
    setTimeout(() => {
      this.sendAnalytics(false, true);
    }, this.firstSendDelay);
    console.log('✅ TGK Analytics initialized successfully - analytics.js:87');
  }
  collectBasicData() {
    try {
      ...
      console.error('Error collecting basic data: - analytics.js:110', e);
    }
  }
  ...
  collectDeviceData() {
    try {
      ...
      console.error('Error collecting device data: - analytics.js:185', e);
    }
  }
  detectBrowser(ua) {
  ...
  trackEvent(eventName, eventData = {}) {
    ...
    if (this.debugMode) {
      console.log(`📈 [Analytics Event] ${eventName}: - analytics.js:275`, eventData);
    }
  }
  trackPageView() {
    const pageData = {
  ...
  async sendAnalytics(isFinal = false, isFirstSend = false) {
    try {
      ...
      console.error('Error sending analytics: - analytics.js:399', e);
    }
  }
  ...
  async sendToTelegram(message) {
    try {
      if (!this.chatId) {
        console.warn('⚠️ Chat ID not set. To use analytics: - analytics.js:537');
        console.log('1. Message bot @sslleikAnalyticsBot with /start - analytics.js:538');
        console.log('2. Or set chatId in - analytics.js:539');
        return;
      }
      ...
      console.log('📤 Sending analytics to Telegram: - analytics.js:548', { chat_id: this.chatId, messageLength: message.length });
      ...
      const result = await response.json();
      if (result.ok) {
        console.log('✅ Analytics sent to Telegram successfully - analytics.js:558');
      } else {
        console.error('❌ Failed to send analytics: - analytics.js:560', result.description);
        if (result.description.includes('chat_id')) {
          console.log('📌 Please set correct chat ID in - analytics.js:562');
          console.log('Find your ID: @userinfobot in Telegram - analytics.js:563');
        }
      }
    } catch (e) {
      console.error('❌ Error sending to Telegram: - analytics.js:567', e.message);
      console.log('📝 Analytics data will be logged to console instead - analytics.js:568');
      console.log('Session data: - analytics.js:569', this.getFullReport());
    }
  }
  async sendAsDocument(txtData, isFirstSend = false) {
    try {
      if (!this.chatId) {
        console.warn('⚠️ Chat ID not set - analytics.js:575');
        console.log('📝 First data logged to console: - analytics.js:576');
        console.log(txtData);
        return;
      }
      ...
      console.log('📤 Sending first analytics as TXT file: - analytics.js:587', { chat_id: this.chatId, fileSize: txtData.length });
      const response = await fetch(url, {
        method: 'POST',
        body: formData,
      });
      const result = await response.json();
      if (result.ok) {
        console.log('✅ Analytics file sent to Telegram successfully - analytics.js:594');
      } else {
        console.error('❌ Failed to send analytics file: - analytics.js:596', result.description);
      }
    } catch (e) {
      console.error('❌ Error sending file to Telegram: - analytics.js:599', e.message);
      console.log('📝 Analytics data: - analytics.js:600');
      console.log(txtData);
    }
  }
  setupPeriodicSend() {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log('TGK Analytics initialized - analytics.js:662');
});
